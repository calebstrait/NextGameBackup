...
def get_game_names():
    ...
    for o in range(0,api_calls):
        ...
        for t in range(0,len(txt)):
            ...
            if len(nam) > 0:
                if len(summ) > 0:
                    if (len(genre) > 0) & (',' not in nam[0]) & (('41' in plts) | ('48' in plts) | ('49' in plts)): # If this game is on PS4, Wii U, or XB1
...
def get_recommendations(Games_DF,platforms,found):
    ...
    nt = namedtuple('nt', ['names','covers','genres','glink','amazn','summary','found'])
    ret = nt(names, covers, genres, glink, amazn, summary, found)
    return ret
